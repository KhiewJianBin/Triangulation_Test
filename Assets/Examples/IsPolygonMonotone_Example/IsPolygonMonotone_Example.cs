...
namespace asim.unity.utils.geometry
{
    public class IsPolygonMonotone_Example : MonoBehaviour
    {
        ...
        public static List<(Vector2,Vector2)> diagonals = new List<(Vector2, Vector2)>();

        (Vector2, Vector2, Vector2) FindAnEar(List<Vector2> GSP,int index)//Good sub polygon
        {
            Vector2 prevVertex = GSP[(int)MathExtensions.fmod(index - 1, GSP.Count)];
            Vector2 currentVertex = GSP[index];
            Vector2 nextVertex = GSP[(int)MathExtensions.fmod(index + 1, GSP.Count)];
            ...
            for (int i = 0; i < GSP.Count; i++)
            {
                Vector2 edgep1 = GSP[(int)MathExtensions.fmod(i, GSP.Count)];
                Vector2 edgep2 = GSP[(int)MathExtensions.fmod(i + 1, GSP.Count)];
            ...
            Vector2 pk1 = GSP[(int)MathExtensions.fmod(kindex + 1, GSP.Count)];
            ...
            if(count>0)
            {
                ...
                {
                    int r = (int)MathExtensions.fmod(kindex + 1 + i, GSP.Count);
                    Vector2 pr = GSP[r];
                ...
                if (zIndex.HasValue && zIndex.Value != index-1)
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        newGSP.Add(GSP[(int)MathExtensions.fmod(pj + i, GSP.Count)]);
                    }
            ...
            else
            {
                ...
                {
                    int s = (int)MathExtensions.fmod(kindex + 1 + i, GSP.Count);
                    Vector2 ps = GSP[s];
            ...
            Vector3 kp1pos = cam.WorldToScreenPoint(GSP[(int)MathExtensions.fmod(kindex + 1, GSP.Count)]);
