        ...
        public static List<Vector2> SimpleEarClipping(SimplePolygon simpleP)
        {
            ...
            List<Vector2> CutAndJoin(List<Vector2> OuterVerts, List<List<Vector2>> HoleVerts)
            {
                while (HoleVerts.Count != 0)
                {
                    ...
                    {
                        Vector2 edgep1 = OuterVerts[(int)MathExtensions.IndexLoop(i, OuterVerts.Count)]; ;
                        Vector2 edgep2 = OuterVerts[(int)MathExtensions.IndexLoop(i + 1, OuterVerts.Count)];
                    ...
                    {
                        OuterVerts.Insert(OuterIndexMaxX_P, Holes[(int)MathExtensions.IndexLoop(InnerIndexMaxX_M - i, Holes.Count)]);
                    }
            ...
            (Vector2, Vector2, Vector2) FindEar(List<Vector2> SPolygon)
            {
                ...
                while (true)
                {
                    int prevIndex = (int)MathExtensions.IndexLoop(i - 1, SPolygon.Count);
                    int currentIndex = i;
                    int nextIndex = (int)MathExtensions.IndexLoop(i + 1, SPolygon.Count);
