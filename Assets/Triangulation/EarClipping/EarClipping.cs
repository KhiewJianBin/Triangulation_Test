...
namespace asim.unity.geometry.triangulation
{
    ...
    {
        /// <summary>
        /// Based on Gary Meisters's the Two-Ears Theorem, where every polygon simple polygon has at least two non-over lapping ears
        /// From a SimplyPolygon, slowly a ear vertex, remove it and repeat
        /// Summerized by :https://www.geometrictools.com/Documentation/TriangulationByEarClipping.pdf
        /// TODO Support Holes, does not support polygon/nested holes
        /// Time complexity : O(n*n)
        public static List<Vector2> SimpleEarClip(SimplePolygon SPolygon)
        {
            var OuterVerts = SPolygon.OuterVertices;
            var HoleVerts = SPolygon.HolesVertices;
            ...
            List<Vector2> CutAndJoin(List<Vector2> OuterVerts, List<List<Vector2>> HoleVerts)
            {
                while (HoleVerts.Count != 0)
                {
                    ...
                    {
                        Vector2 edgep1 = OuterVerts[(int)MathExtensions.fmod(i, OuterVerts.Count)]; ;
                        Vector2 edgep2 = OuterVerts[(int)MathExtensions.fmod(i + 1, OuterVerts.Count)];
                    ...
                    {
                        OuterVerts.Insert(OuterIndexMaxX_P, Holes[(int)MathExtensions.fmod(InnerIndexMaxX_M - i, Holes.Count)]);
                    }
